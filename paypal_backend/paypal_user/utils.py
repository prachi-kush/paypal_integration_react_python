...
def make_paypal_payment(amount, currency, return_url, cancel_url):
    ...
    try:
        ...
        payment_response.raise_for_status()  # Raise HTTPError for bad responses
    except requests.exceptions.RequestException as e:
...
def verify_paypal_payment(payment_id):
    ...
    token_url = base_url + '/v1/oauth2/token'
    payment_url = base_url + '/v1/payments/payment'
    # Print credentials to debug (remove in production)
    ...
    try:
        ...
        payment_details_response.raise_for_status()  # Raise HTTPError for bad responses
    except requests.exceptions.RequestException as e:
        print(f"Error retrieving PayPal payment details: {e}")
